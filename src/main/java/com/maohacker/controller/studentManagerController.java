...
import com.maohacker.service.studentManagerService;
...
@RequestMapping("/a/profession")
public class studentManagerController {
    private static Logger logger = Logger.getLogger(studentManagerController.class);  //在类的成员变量处声明
    @Autowired//自动把bean里面引用的对象的setter/getter方法省略
    @Qualifier("studentManagerService")//进行自动注入
    private studentManagerService studentManagerService;

    @SuppressWarnings("all")
    /**
     * 返回学生信息接口
     * 调用接口示例:http://localhost/face-admin-ajax/a/profession/search?page=1&size=10
     * 请求方式:GET
     * @param response
     * @return Map
     */
    ...
    public Map<String, Object> qryUniversityInfo(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            reMap = studentManagerService.qryUniversityInfo(paramMap);
        } catch (Exception e) {
